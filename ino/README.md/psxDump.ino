...
void loop () {
	...
	if (!haveController) {
      ...
      uint16_t word = psx.getButtonWord ();
      if (old_word != 0 || word != 0) {
        old_word = word;
        
        uint8_t firstByte = uint8_t(word);        
        uint8_t secondByte = uint8_t(word >> 8); 
        if(firstByte == 0) {
            firstByte = 0x3;          
        }
        if(secondByte == 0) {
        ...
        uint8_t leftDigitalData[] = {0xf2, firstByte, secondByte};
        char leftDigitalString[sizeof(leftDigitalData)*2] = {0};
        memcpy(leftDigitalString, leftDigitalData, sizeof(leftDigitalData));        
        Serial.println (leftDigitalString);
			...
			if (lx != slx || ly != sly) {
        uint8_t size = 2;        
        uint8_t leftAnalogData[] = {0xf1, lx, ly};
        char leftAnalogString[sizeof(leftAnalogData)*2] = {0};
        memcpy(leftAnalogString, leftAnalogData, sizeof(leftAnalogData));        
				Serial.println (leftAnalogString);
				slx = lx;
				sly = ly;
			}
			...
			if (rx != srx || ry != sry) {
        uint8_t rightAnalogData[3] = {0xf0, rx, ry};
        char rightAnalogString[6] = {0};
        memcpy(rightAnalogString, rightAnalogData, 3);        
				Serial.println (rightAnalogString);
				srx = rx;
				sry = ry;
			}
		}
